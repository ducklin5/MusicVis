...
draw = function () {
	...
	starfield.origin = createVector(1000,random(-height,height),random(-height/1.5,1000));
	starfield.shape = cylinder;
	starfield.color = [random(0,255),115,180];
	starfield.size = 2;
	starfield.lifespan = 200;
	starfield.accelaration = createVector(-10,0,0);
	starfield.velocity= createVector(-10,0,0);
	...
	this.draw = function () {
		...
		for(var i = min; i <= max; i += inc){
			...
			if (this.style == 'spiral'){
				var gamma = map(i, min, max, 0, 1.5*PI) + this.spiralAngle;
...
var Particle = function (origin,shape,size,color,lifespan,accelaration,velocity) {
	...
	this.draw = function () {
		basicMaterial(this.color[0],this.color[1],this.color[2]);
